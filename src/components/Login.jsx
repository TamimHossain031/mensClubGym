...
import { anotherLogin, signIn } from "./Utility/firebase";
export default function Login() {
  ...
  const { isLogin, addV } = useContext(login);
      ...
      >
        ...
        {error && (
          <p className="text-xs text-red-400 py-1">
            Your admin password is Wrong{" "}
            <button className="text-white underline" onClick={handleModal}>
              Update PassWord
            </button>
          </p>
        )}
        <label htmlFor="admin">
          <input
            ...
            type="checkbox"
            checked={isAdmin}
            onChange={() => setIsAdmin((single) => !single)}
          />{" "}
          Login as Admin
        </label>
