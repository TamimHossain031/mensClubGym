  ...
  return (
    <nav className="px-2 py-5 flex justify-between relative z-10">
      ...
      {show &&  (
        ...
        { user && <ul className="space-y-3 text-center mt-3 w-[100%]">
          ...
          <button className=' border mt-4  px-2 py-1 rounded' onClick={()=> user ? signOut() : navigate('login')}>{user ? 'logout' : 'login'}</button>
        </div>
      )}
    </nav>
  );
